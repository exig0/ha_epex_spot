...
class SourceShell:
    def __init__(self, config_entry: ConfigEntry, session: aiohttp.ClientSession):
        ...
        elif config_entry.data[CONF_SOURCE] == CONF_SOURCE_ELEX:
            self._source = ELEX.Elex(
                market_area=config_entry.data[CONF_MARKET_AREA],
                api_key=config_entry.data[CONF_TOKEN],
                duration=config_entry.options.get(CONF_DURATION, DEFAULT_DURATION), # <--- ADD THIS LINE
                session=session,
            )
        else:
